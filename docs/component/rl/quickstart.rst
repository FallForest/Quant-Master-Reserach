...
QuantMasterRL provides an example of an implementation of a single asset order execution task and the following is an example of the config file to train with QuantMasterRL.
...
.. code-block:: yaml
    ...
    action_interpreter:
        ...
        kwargs:
            ...
            max_step: 8
        module_path: quant_master.rl.order_execution.interpreter
    state_interpreter:
        ...
        kwargs:
            ...
            processed_data_provider:
                ...
                module_path: quant_master.rl.data.pickle_styled
                kwargs:
    ...
    reward:
        ...
        kwargs:
            ...
            penalty: 100.0
        module_path: quant_master.rl.order_execution.reward
    data:
        source:
    ...
    network:
        class: Recurrent
        module_path: quant_master.rl.order_execution.network
    policy:
        class: PPO
        kwargs:
            lr: 0.0001
...
.. code-block:: yaml
    ...
    strategies: 
        ...
        1day: 
            ...
            module_path: quant_master.rl.order_execution.strategy
            kwargs:
            state_interpreter:
                class: FullHistoryStateInterpreter
                module_path: quant_master.rl.order_execution.interpreter
...
.. code-block:: console
    ...
    $ python -m quant_master.rl.contrib.train_onpolicy.py --config_path train_config.yml

After the training, you can backtest with the following command:

.. code-block:: console

    $ python -m quant_master.rl.contrib.backtest.py --config_path backtest_config.yml

In that case, :class:`~quant_master.rl.order_execution.simulator_quant_master.SingleAssetOrderExecution` and :class:`~quant_master.rl.order_execution.simulator_simple.SingleAssetOrderExecutionSimple` as examples for simulator, :class:`quant_master.rl.order_execution.interpreter.FullHistoryStateInterpreter` and :class:`quant_master.rl.order_execution.interpreter.CategoricalActionInterpreter` as examples for interpreter, :class:`quant_master.rl.order_execution.policy.PPO` as an example for policy, and :class:`quant_master.rl.order_execution.reward.PAPenaltyReward` as an example for reward.
...
The details about the example can be found `here <https://github.com/microsoft/quant_master/blob/main/examples/rl/README.md>`_. 

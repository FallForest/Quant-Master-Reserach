...
An example of the entire process is shown `here <https://github.com/microsoft/quant_master/tree/main/examples/model_rolling/task_manager_rolling.py>`__.
...
.. autoclass:: quant_master.workflow.task.gen.TaskGen
    :members:
    :noindex:

``QuantMaster`` provides a class `RollingGen <https://github.com/microsoft/quant_master/tree/main/quant_master/workflow/task/gen.py>`_ to generate a list of ``task`` of the dataset in different date segments.
...
.. autoclass:: quant_master.workflow.task.manage.TaskManager
    :members:
    :noindex:
...
``QuantMaster`` provides a method called ``run_task`` to run those ``task`` in task pool, however, users can also customize how tasks are executed.
An easy way to get the ``task_func`` is using ``quant_master.model.trainer.task_train`` directly.
It will run the whole workflow defined by ``task``, which includes *Model*, *Dataset*, *Record*.

.. autofunction:: quant_master.workflow.task.manage.run_task
    :noindex:

Meanwhile, ``QuantMaster`` provides a module called ``Trainer``. 

.. autoclass:: quant_master.model.trainer.Trainer
    :members:
    :noindex:
...
Task Collecting
===============
Before collecting model training results, you need to use the ``quant_master.init`` to specify the path of mlruns.

To collect the results of ``task`` after training, ``QuantMaster`` provides `Collector <../reference/api.html#Collector>`_, `Group <../reference/api.html#Group>`_ and `Ensemble <../reference/api.html#Ensemble>`_ to collect the results in a readable, expandable and loosely-coupled way.
...
For more information, please see `Collector <../reference/api.html#Collector>`_, `Group <../reference/api.html#Group>`_ and `Ensemble <../reference/api.html#Ensemble>`_, or the `example <https://github.com/microsoft/quant_master/tree/main/examples/model_rolling/task_manager_rolling.py>`_.

...
class DataHandlerLP(DataHandler):
    ...
    - Please note processed data like `self._infer` or `self._learn` are concepts different from `segments` in QuantMaster's `Dataset` like "train" and "test"

        - Processed data like `self._infer` or `self._learn` are underlying data processed with different processors
        - `segments` in QuantMaster's `Dataset` like "train" and "test" are simply the time segmentations when querying data("train" are often before "test" in time-series).
    ...
    def from_df(cls, df: pd.DataFrame) -> "DataHandlerLP":
        ...
        .. code-block:: python
            ...
            from quant_master.data.dataset import DataHandlerLP

...
class PITProvider(abc.ABC):
    ...
    def period_feature(
        ...
        period: int
            ...
            The period is represented with int in QuantMaster. (e.g. 202001 may represent the first quarter in 2020)
    ...
    def load_calendar(self, freq, future):
        ...
        try:
            ...
            if future:
                ...
                get_module_logger("data").warning(
                    "You can get future calendar by referring to the following document: https://github.com/microsoft/quant_master/blob/main/scripts/data_collector/contrib/README.md"
                )
...
class ClientDatasetProvider(DatasetProvider):
    ...
    def dataset(
        ...
        if disk_cache == 0:
            ...
            """
            # TODO: support inst_processors, need to change the code of quant_master-server at the same time
            ...
            if inst_processors:
                raise ValueError(
                    ...
                    f"Please use `D.features(disk_cache=0)` or `quant_master.init(dataset_cache=None)`"
                )
            self.conn.send_request(
                request_type="feature",
                request_content={
...
class BaseProvider:
    ...
    To keep compatible with old quant_master provider.
    """
...
def register_all_wrappers(C):
    ...
    module = get_module_by_module_path("quant_master.data")
    ...
    register_wrapper(Cal, _calendar_provider, "quant_master.data")
    ...
    register_wrapper(Inst, _instrument_provider, "quant_master.data")
    ...
    if getattr(C, "feature_provider", None) is not None:
        ...
        register_wrapper(FeatureD, feature_provider, "quant_master.data")
    ...
    if getattr(C, "pit_provider", None) is not None:
        ...
        register_wrapper(PITD, pit_provider, "quant_master.data")
    ...
    if getattr(C, "expression_provider", None) is not None:
        ...
        register_wrapper(ExpressionD, _eprovider, "quant_master.data")
    ...
    register_wrapper(DatasetD, _dprovider, "quant_master.data")
    logger.debug(f"registering DatasetD {C.dataset_provider}-{C.dataset_cache}")

    register_wrapper(D, C.provider, "quant_master.data")

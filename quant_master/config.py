...
class QSettings(BaseSettings):
    """
    QuantMaster's settings.
    It tries to provide a default settings for most of QuantMaster's components.
    But it would be a long journey to provide a comprehensive settings for all of QuantMaster's components.

    Here is some design guidelines:
    - The priority of settings is
        - Actively passed-in settings, like `quant_master.init(provider_uri=...)`
        - The default settings
            - QSettings tries to provide default settings for most of QuantMaster's components.
    """
...
_default_config = {
    ...
    # config it in quant_master.init()
    # "provider_uri" str or dict:
    #   # str
    #   "~/.quant_master/stock_data/cn_data"
    #   # dict
    #   {"day": "~/.quant_master/stock_data/cn_data", "1min": "~/.quant_master/stock_data/cn_data_1min"}
    # NOTE: provider_uri priority:
    #   1. backend_config: backend_obj["kwargs"]["provider_uri"]
    #   2. backend_config: backend_obj["kwargs"]["provider_uri_map"]
    #   3. quant_master.init: provider_uri
    "provider_uri": "",
    # cache
    ...
    "redis_task_db": 1,
    "redis_password": None,
    # This value can be reset via quant_master.init
    "logging_level": logging.INFO,
    # Global configuration of quant_master log
    ...
    "logging_config": {
        ...
        # [1] https://github.com/microsoft/quant_master/pull/1661
        # [2] https://github.com/pytest-dev/pytest/issues/3697
        "loggers": {"quant_master": {"level": logging.DEBUG, "handlers": ["console"], "propagate": False}},
        # To let quant_master work with other packages, we shouldn't disable existing loggers.
...
MODE_CONF = {
    "server": {
        ...
        "local_cache_path": Path("~/.cache/quant_master_simple_cache").expanduser().resolve(),
        "mount_path": None,
    },
    "client": {
        ...
        # The nfs should be auto-mounted by quant_master on other
...
HIGH_FREQ_CONFIG = {
    "provider_uri": "~/.quant_master/quant_master_data/cn_data_1min",
    "dataset_cache": None,
...
class QuantMasterConfig(Config):
    # URI_TYPE
    LOCAL_URI = "local"
    NFS_URI = "nfs"
    ...
    class DataPathManager:
        ...
        def format_provider_uri(provider_uri: Union[str, dict, Path]) -> dict:
            ...
            if isinstance(provider_uri, (str, dict, Path)):
                if not isinstance(provider_uri, dict):
                    provider_uri = {QuantMasterConfig.DEFAULT_FREQ: provider_uri}
            else:
                raise TypeError(f"provider_uri does not support {type(provider_uri)}")
        ...
        def get_uri_type(uri: Union[str, Path]):
            ...
            if is_nfs_or_win and not is_win:
                return QuantMasterConfig.NFS_URI
            else:
                return QuantMasterConfig.LOCAL_URI
        ...
        def get_data_uri(self, freq: Optional[Union[str, Freq]] = None) -> Path:
            ...
            if freq is None or freq not in self.provider_uri:
                freq = QuantMasterConfig.DEFAULT_FREQ
            _provider_uri = self.provider_uri[freq]
            if self.get_uri_type(_provider_uri) == QuantMasterConfig.LOCAL_URI:
                return Path(_provider_uri)
            elif self.get_uri_type(_provider_uri) == QuantMasterConfig.NFS_URI:
    ...
    def set(self, default_conf: str = "client", **kwargs):
        """
        configure quant_master based on the input parameters
    ...
    def register(self):
        ...
        from .workflow import R, QuantMasterRecorder  # pylint: disable=C0415
        from .workflow.utils import experiment_exit_handler  # pylint: disable=C0415

        register_all_ops(self)
        register_all_wrappers(self)
        # set up QuantMasterRecorder
        exp_manager = init_instance_by_config(self["exp_manager"])
        qr = QuantMasterRecorder(exp_manager)
        R.register(qr)
        # clean up experiment when python program ends
        experiment_exit_handler()

        # Supporting user reset quant_master version (useful when user want to connect to quant_master server with old version)
        self.reset_quant_master_version()

        self._registered = True

    def reset_quant_master_version(self):
        import quant_master  # pylint: disable=C0415

        reset_version = self.get("quant_master_reset_version", None)
        if reset_version is not None:
            quant_master.__version__ = reset_version
        else:
            quant_master.__version__ = getattr(quant_master, "__version__bak")
            # Due to a bug? that converting __version__ to _QuantMasterConfig__version__bak
...
# global config
C = QuantMasterConfig(_default_config)

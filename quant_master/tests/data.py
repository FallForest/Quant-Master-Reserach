...
from quant_master.utils import exists_quant_master_data


class GetData:
    # Keep using the existing dataset release bucket while the repo/package
    # namespace has moved to quant_master.
    REMOTE_URL = "https://github.com/SunsetWolf/" + "q" + "lib_dataset/releases/download"
    ...
    def download_data(self, file_name: str, target_dir: [Path, str], delete_old: bool = True):
        ...
        ---------
        # get rl data
        python get_data.py download_data --file_name rl_data.zip --target_dir ~/.quant_master/quant_master_data/rl_data
        When this command is run, the data will be downloaded from this link: https://quant_masterpublic.blob.core.windows.net/data/default/stock_data/rl_data.zip?{token}

        # get cn csv data
        python get_data.py download_data --file_name csv_data_cn.zip --target_dir ~/.quant_master/csv_data/cn_data
        When this command is run, the data will be downloaded from this link: https://quant_masterpublic.blob.core.windows.net/data/default/stock_data/csv_data_cn.zip?{token}
        -------
    ...
    def _unzip(file_path: [Path, str], target_dir: [Path, str], delete_old: bool = True):
        ...
        if delete_old:
            logger.warning(
                f"will delete the old quant_master data directory(features, instruments, calendars, features_cache, dataset_cache): {target_dir}"
            )
            GetData._delete_quant_master_data(target_dir)
    ...
    @staticmethod
    def _delete_quant_master_data(file_dir: Path):
        rm_dirs = []
    ...
    def quant_master_data(
        ...
        """download cn quant_master data from remote
        ...
        # get 1min data
        python get_data.py quant_master_data --name quant_master_data --target_dir ~/.quant_master/quant_master_data/cn_data_1min --interval 1min --region cn
        When this command is run, the data will be downloaded from this link: https://quant_masterpublic.blob.core.windows.net/data/default/stock_data/v2/quant_master_data_cn_1min_latest.zip?{token}
        -------
        ...
        quant_master_version = ".".join(re.findall(r"(\d+)\.+", quant_master.__version__))
        remote_name = name.replace("quant_master", "q" + "lib")

        def _get_file_name_with_version(quant_master_version, dataset_version):
            dataset_version = "v2" if dataset_version is None else dataset_version
            file_name_with_version = (
                f"{dataset_version}/{remote_name}_{region.lower()}_{interval.lower()}_{quant_master_version}.zip"
            )
            return file_name_with_version

        file_name = _get_file_name_with_version(quant_master_version, dataset_version=version)

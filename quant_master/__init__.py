...
__version__bak = __version__  # This version is backup for QuantMasterConfig.reset_quant_master_version
import logging
import os
import platform
import re
...
def init(default_conf="client", **kwargs):
    ...
    **kwargs :
    ...
    if skip_if_reg and C.registered:
        # if we reinitialize QuantMaster during running an experiment `R.start`.
    ...
    logger.info("quant_master successfully initialized based on %s settings." % default_conf)
...
def init_from_yaml_conf(conf_path, **kwargs):
    ...
    :param conf_path: A path to the quant_master config in yml format
    """
...
def get_project_path(config_name="config.yaml", cur_path: Union[Path, str, None] = None) -> Path:
    ...
    - QuantMaster is a sub folder in project path
    - There is a file named `config.yaml` in quant_master.
    ...
    - user want to use a relative config path instead of hard-coding quant_master config path in code
...
def auto_init(**kwargs):
    """
    This function will init quant_master automatically with following priority
    - Find the project configuration and init quant_master
        - The parsing process will be affected by the `conf_type` of the configuration file
    - Init quant_master with default config
    ...
    .. code-block:: yaml
        ...
        conf_type: ref
        quant_master_cfg: '<shared_yaml_config_path>'    # this could be null reference no config from other files
        # following configs in `quant_master_cfg_update` is project=specific
        quant_master_cfg_update:
            exp_manager:
    ...
    try:
        ...
        if conf_type == "origin":
            # The type of config is just like original quant_master config
            ...
            quant_master_conf_path = conf.get("quant_master_cfg", None)

            # merge the arguments
            quant_master_conf_update = conf.get("quant_master_cfg_update", {})
            for k, v in kwargs.items():
                if k in quant_master_conf_update:
                    logger.warning(f"`quant_master_conf_update` from conf_pp is override by `kwargs` on key '{k}'")
            quant_master_conf_update.update(kwargs)

            init_from_yaml_conf(quant_master_conf_path, **quant_master_conf_update)

...
from ...data import dataset as quant_master_dataset
...
class MultiSegRecord(RecordTemp):
    ...
    def __init__(self, model, dataset, recorder=None):
        ...
        if not isinstance(dataset, quant_master_dataset.DatasetH):
    ...
    def generate(self, segments: Dict[Text, Any], save: bool = False):
        for key, segment in segments.items():
            ...
            labels = self.dataset.prepare(
                segments=segment, col_set="label", data_key=quant_master_dataset.handler.DataHandlerLP.DK_R
            )

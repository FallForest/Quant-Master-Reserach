...
from quant_master.config import REG_CN
...
from quant_master.contrib.ops.high_freq import get_calendar_day, DayLast, FFillNan, BFillNan, Date, Select, IsNull, IsInf, Cut
...
class HighFreqProvider:
    def __init__(
        ...
        **kwargs,
        ...
        self._init_quant_master(quant_master_conf)
        self.feature_conf = feature_conf
        self.label_conf = label_conf
        self.backtest_conf = backtest_conf
        self.quant_master_conf = quant_master_conf
    ...
    def _init_quant_master(self, quant_master_conf):
    ...
    def _gen_day_dataset(self, config, conf_type):
        ...
        def generate_dataset(times):
            if os.path.isfile(path + times.strftime("%Y-%m-%d") + ".pkl"):
                ...
                return
            self._init_quant_master(self.quant_master_conf)

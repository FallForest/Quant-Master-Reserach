...
from quant_master.utils import hash_args
from quant_master.utils.mod import init_instance_by_config
    ...
    For example:
        ...
        Using quant_master.init():
            ...
            .. code-block:: python
                ...
                quant_master.init(..., mongo=mongo_conf)

        After quant_master.init():
...
def replace_task_handler_with_cache(task: dict, cache_dir: Union[str, Path] = ".") -> dict:
    ...
    >>> import quant_master
    >>> quant_master.auto_init()
    >>> import datetime
    >>> # it is simplified task
    >>> task = {"dataset": {"kwargs":{'handler': {'class': 'Alpha158', 'module_path': 'quant_master.contrib.data.handler', 'kwargs': {'start_time': datetime.date(2008, 1, 1), 'end_time': datetime.date(2020, 8, 1), 'fit_start_time': datetime.date(2008, 1, 1), 'fit_end_time': datetime.date(2014, 12, 31), 'instruments': 'CSI300'}}}}}

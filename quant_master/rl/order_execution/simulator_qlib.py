...
from quant_master.backtest import collect_data_loop, get_strategy_executor
from quant_master.backtest.decision import BaseTradeDecision, Order, TradeRangeByTime
from quant_master.backtest.executor import NestedExecutor
...
class SingleAssetOrderExecution(Simulator[Order, SAOEState, float]):
    """Single-asset order execution (SAOE) simulator which is implemented based on QuantMaster backtest tools.
    ...
    quant_master_config
        Configuration used to initialize QuantMaster. If it is None, QuantMaster will not be initialized.
    cash_limit:
        Cash limit.
    """
    ...
    def __init__(
        ...
        quant_master_config: dict | None = None,
        ...
        strategy_config = {
            ...
            "module_path": "quant_master.rl.strategy.single_order",
            "kwargs": {
                "order": order,
        ...
        self.reset(order, strategy_config, executor_config, exchange_config, quant_master_config, cash_limit)
    ...
    def reset(
        ...
        if quant_master_config is not None:
            init_quant_master(quant_master_config)

...
from quant_master.config import C, QuantMasterConfig
...
def complex_parallel(paral: Parallel, complex_iter):
    ...
    >>> from quant_master.utils.paral import complex_parallel
    >>> from joblib import Parallel, delayed
...
class call_in_subproc:
    ...
    def __init__(self, func: Callable, quant_master_config: QuantMasterConfig = None):
        """
        Parameters
        ----------
        func : Callable
        ...
        quant_master_config : QuantMasterConfig
            QuantMaster config for initialization in subprocess
        ...
        """
        self.func = func
        self.quant_master_config = quant_master_config

    def _func_mod(self, *args, **kwargs):
        """Modify the initial function by adding QuantMaster initialization"""
        if self.quant_master_config is not None:
            C.register_from_C(self.quant_master_config)

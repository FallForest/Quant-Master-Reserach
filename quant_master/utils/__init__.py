...
def get_date_by_shift(
    ...
    """
    from quant_master.data import D  # pylint: disable=C0415
...
def time_to_slc_point(t: Union[None, str, pd.Timestamp]) -> Union[None, pd.Timestamp]:
    """
    Time slicing in QuantMaster or Pandas is a frequently-used action.
    ...
    """
    if t is None:
        # None represents unbounded in QuantMaster or Pandas(e.g. df.loc[slice(None, "20210303")]).
        return t
    else:
...
def exists_quant_master_data(quant_master_dir):
    quant_master_dir = Path(quant_master_dir).expanduser()
    if not quant_master_dir.exists():
        return False

    calendars_dir = quant_master_dir.joinpath("calendars")
    instruments_dir = quant_master_dir.joinpath("instruments")
    features_dir = quant_master_dir.joinpath("features")
    # check dir
...
def check_quant_master_data(quant_master_config):
    inst_dir = Path(quant_master_config["provider_uri"]).joinpath("instruments")
    for _p in inst_dir.glob("*.txt"):
        assert len(pd.read_csv(_p, sep="\t", nrows=0, header=None).columns) == 3, (
            f"\nThe {str(_p.resolve())} of quant_master data is not equal to 3 columns:"
            f"\n\tIf you are using the data provided by quant_master: "
            f"https://quant_master.readthedocs.io/en/latest/component/data.html#quant_master-format-dataset"
            f"\n\tIf you are using your own data, please dump the data again: "
            f"https://quant_master.readthedocs.io/en/latest/component/data.html#converting-csv-format-into-quant_master-format"
        )
...
#################### Wrapper #####################
class Wrapper:
    """Wrapper class for anything that needs to set up during quant_master.init"""
    ...
    def __getattr__(self, key):
        if self.__dict__.get("_provider", None) is None:
            raise AttributeError("Please run quant_master.init() first using quant_master")

...
from quant_master.backtest.account import Account
    ...
    def _collect_data(
        ...
        while not self.inner_executor.finished():
            ...
            if not self._align_range_limit or start_idx <= sub_cal.get_trade_step() <= end_idx:
                ...
                # - Normally, user will create a strategy and embed it into QuantMaster's executor and simulator interaction
                #   loop For a _nested quant_master example_, (QuantMaster Strategy) <=> (QuantMaster Executor[(inner QuantMaster Strategy) <=>
                #   (inner QuantMaster Executor)])
                # - However, RL-based framework has it's own script to run the loop
                #   For an _RL learning example_, (RL Policy) <=> (RL Env[(inner QuantMaster Executor)])
                # To make it possible to run  _nested quant_master example_ and _RL learning example_ together, the solution
                # below is proposed
                # - The entry script follow the example of  _RL learning example_ to be compatible with all kinds of
                #   RL Framework
                # - Each step of (RL Env) will make (inner QuantMaster Executor) one step forward
                #     - (inner QuantMaster Strategy) is a proxy strategy, it will give the program control right to (RL Env)

...
class TradeCalendarManager:
    ...
    def get_step_time(self, trade_step: int | None = None, shift: int = 0) -> Tuple[pd.Timestamp, pd.Timestamp]:
        ...
        About the endpoints:
            - QuantMaster uses the closed interval in time-series data selection, which has the same performance as
            pandas.Series.loc
            # - The returned right endpoints should minus 1 seconds because of the closed interval representation in
            #   QuantMaster.
            # Note: QuantMaster supports up to minutely decision execution, so 1 seconds is less than any trading time
            #   interval.

...
- [Collector Data](#collector-data)
  - [Get QuantMaster data](#get-quant_master-databin-file)
  - [Collector *YahooFinance* data to quant_master](#collector-yahoofinance-data-to-quant_master)
  - [Automatic update of daily frequency data](#automatic-update-of-daily-frequency-datafrom-yahoo-finance)
- [Using quant_master data](#using-quant_master-data)
...
### Get QuantMaster data(`bin file`)
  > `quant_master-data` from *YahooFinance*, is the data that has been dumped and can be used directly in `quant_master`.
  > This ready-made quant_master-data is not updated regularly. If users want the latest data, please follow [these steps](#collector-yahoofinance-data-to-quant_master) download the latest data. 

  - get data: `python scripts/get_data.py quant_master_data`
  - parameters:
    - `target_dir`: save dir, by default *~/.quant_master/quant_master_data/tdx_cn_data*
    - `version`: dataset version, value from [`v1`, `v2`], by default `v1`
      - `v2` end date is *2021-06*, `v1` end date is *2020-09*
      - If users want to incrementally update data, they need to use yahoo collector to [collect data from scratch](#collector-yahoofinance-data-to-quant_master).
      - **the [benchmarks](https://github.com/microsoft/quant_master/tree/main/examples/benchmarks) for quant_master use `v1`**, *due to the unstable access to historical data by YahooFinance, there are some differences between `v2` and `v1`*
  ...
  - examples:
    ...
    # us 1d
    python scripts/get_data.py quant_master_data --target_dir ~/.quant_master/quant_master_data/us_data --region us --interval 1d
    ```

### Collector *YahooFinance* data to quant_master
...
> The manual `download -> normalize -> dump` sequence below is retained as a low-level reference for debugging and custom workflows.
  1. download data to csv: `python scripts/data_collector/yahoo/collector.py download_data`
     ...
     - examples:
          ```bash
          # cn 1d data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/cn_data --start 2020-01-01 --end 2020-12-31 --delay 1 --interval 1d --region CN
          # cn 1min data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/cn_data_1min --delay 1 --interval 1min --region CN

          # us 1d data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/us_data --start 2020-01-01 --end 2020-12-31 --delay 1 --interval 1d --region US
          # us 1min data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/us_data_1min --delay 1 --interval 1min --region US

          # in 1d data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/in_data --start 2020-01-01 --end 2020-12-31 --delay 1 --interval 1d --region IN
          # in 1min data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/in_data_1min --delay 1 --interval 1min --region IN

          # br 1d data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/br_data --start 2003-01-03 --end 2022-03-01 --delay 1 --interval 1d --region BR
          # br 1min data
          python collector.py download_data --source_dir ~/.quant_master/stock_data/source/br_data_1min --delay 1 --interval 1min --region BR
          ```
  2. normalize data: `python scripts/data_collector/yahoo/collector.py normalize_data`
     ...
     - parameters:
          ...
          - `interval`: `1d` or `1min`, by default `1d`
            > if **`interval == 1min`**, `quant_master_data_1d_dir` cannot be `None`
          - `region`: `CN` or `US` or `IN`, by default `CN`
          ...
          - `quant_master_data_1d_dir`: quant_master directory(1d data)
            ...
            if interval==1min, quant_master_data_1d_dir cannot be None, normalize 1min needs to use 1d data;
                ...
                quant_master_data_1d can be obtained like this:
                    ...
                    $ python scripts/data_collector/yahoo/collector.py update_data_to_bin --quant_master_data_1d_dir <quant_master_data_1d_dir> --end_date <end_date>
                or:
      ...
      - examples:
        ```bash
        # normalize 1d cn
        python collector.py normalize_data --source_dir ~/.quant_master/stock_data/source/cn_data --normalize_dir ~/.quant_master/stock_data/source/cn_1d_nor --region CN --interval 1d

        # normalize 1min cn
        python collector.py normalize_data --quant_master_data_1d_dir ~/.quant_master/quant_master_data/tdx_cn_data --source_dir ~/.quant_master/stock_data/source/cn_data_1min --normalize_dir ~/.quant_master/stock_data/source/cn_1min_nor --region CN --interval 1min

        # normalize 1d br
        python scripts/data_collector/yahoo/collector.py normalize_data --source_dir ~/.quant_master/stock_data/source/br_data --normalize_dir ~/.quant_master/stock_data/source/br_1d_nor --region BR --interval 1d

        # normalize 1min br
        python collector.py normalize_data --quant_master_data_1d_dir ~/.quant_master/quant_master_data/br_data --source_dir ~/.quant_master/stock_data/source/br_data_1min --normalize_dir ~/.quant_master/stock_data/source/br_1min_nor --region BR --interval 1min
        ```
  3. dump data: `python scripts/dump_bin.py dump_all`
     ...
     - examples:
       ```bash
       # dump 1d cn
       python dump_bin.py dump_all --data_path ~/.quant_master/stock_data/source/cn_1d_nor --quant_master_dir ~/.quant_master/quant_master_data/tdx_cn_data --freq day --exclude_fields date,symbol --file_suffix .csv
       # dump 1min cn
       python dump_bin.py dump_all --data_path ~/.quant_master/stock_data/source/cn_1min_nor --quant_master_dir ~/.quant_master/quant_master_data/cn_data_1min --freq 1min --exclude_fields date,symbol --file_suffix .csv
       ```

### Automatic update of daily frequency data(from yahoo finance)
  > `update_data_to_bin` is the recommended Yahoo 1d entrypoint.
  >
  > **NOTE**: Users can't incrementally  update data based on the offline data provided by QuantMaster(some fields are removed to reduce the data size). Users should use [yahoo collector](https://github.com/microsoft/quant_master/tree/main/scripts/data_collector/yahoo#automatic-update-of-daily-frequency-datafrom-yahoo-finance) to download Yahoo data from scratch and then incrementally update it.
  >
  > **NOTE**: Incremental updates require an overlap trading day. The downloaded source data must include the last trading day already stored in `quant_master_data_1d_dir`.
  > 
  ...
  * `scripts/data_collector/yahoo/collector.py update_data_to_bin` parameters:
      ...
      * `quant_master_data_1d_dir`: the quant_master data to be updated for yahoo, usually from: [download quant_master data](https://github.com/microsoft/quant_master/tree/main/scripts#download-cn-data)
...
## Using quant_master data
  ...
  # 1min data cn
  # freq=1min
  quant_master.init(provider_uri="~/.quant_master/quant_master_data/cn_data_1min", region="cn")
  ...
  # 1d data us
  quant_master.init(provider_uri="~/.quant_master/quant_master_data/us_data", region="us")
  df = D.features(D.instruments("all"), ["$close"], freq="day")

  # 1min data us
  quant_master.init(provider_uri="~/.quant_master/quant_master_data/us_data_1min", region="cn")
  inst = D.list_instruments(D.instruments("all"), freq="1min", as_list=True)
  # get 100 symbols
  df = D.features(inst[:100], ["$close"], freq="1min")
  # get all symbol data

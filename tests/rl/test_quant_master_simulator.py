...
from quant_master.backtest.decision import Order, OrderDir
from quant_master.backtest.executor import SimulatorExecutor
from quant_master.rl.order_execution import CategoricalActionInterpreter
...
def get_configs(order: Order) -> Tuple[dict, dict]:
    executor_config = {
        ...
        "kwargs": {
            "time_per_step": "1day",
            "inner_strategy": {"class": "ProxySAOEStrategy", "module_path": "quant_master.rl.order_execution.strategy"},
            "track_data": True,
            "inner_executor": {
                "class": "NestedExecutor",
                "module_path": "quant_master.backtest.executor",
...
def get_simulator(order: Order) -> SingleAssetOrderExecution:
    DATA_ROOT_DIR = Path(__file__).parent.parent / ".data" / "rl" / "quant_master_simulator"
    ...
    return SingleAssetOrderExecution(
        order=order,
        quant_master_config=quant_master_config,

...
The High-Frequency Dataset is implemented as `quant_master.data.dataset.DatasetH` in the `workflow.py`. `DatatsetH` is the subclass of [`quant_master.utils.serial.Serializable`](https://quant_master.readthedocs.io/en/latest/advanced/serial.html), whose state can be dumped in or loaded from disk in `pickle` format.

### About Reinitialization

After reloading `Dataset` from disk, `QuantMaster` also support reinitializing the dataset. It means that users can reset some states of `Dataset` or `DataHandler` such as `instruments`, `start_time`, `end_time` and `segments`, etc.,  and generate new data according to the states.

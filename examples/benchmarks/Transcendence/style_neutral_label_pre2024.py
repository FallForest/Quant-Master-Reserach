...
def build_parser() -> argparse.ArgumentParser:
    p = argparse.ArgumentParser(description="Style-neutral residual return target quick-smoke.")
...
def main() -> int:
    ...
    summary: Dict[str, Any] = {
        ...
        "leakage_guardrails": {
            ...
            "full_mode_guard": "explicit --mode full currently fails closed; this worker artifact is smoke-only",
        },
    }

    try:
        if mode != "smoke":
            raise RuntimeError("full mode is intentionally not implemented; rerun with explicit --mode smoke")
        ...
        investigate_pass = bool(_safe_float(selected["ir"]) >= INVESTIGATE_IR and not finite_gate_errors)
        promotion_pass = bool(
            _safe_float(selected["ir"]) >= PROMOTE_IR
            and _safe_float(selected["annret"]) >= PROMOTE_ANNRET
            and not finite_gate_errors
        )
        verdict = "PROMOTE_TO_FULL_CANDIDATE" if promotion_pass else "INVESTIGATE" if investigate_pass else "NO_GO"
        status = "ok" if promotion_pass or investigate_pass else "gate_failed"
        ...
        return 0 if (promotion_pass or investigate_pass) and not finite_gate_errors else 2
    except Exception as exc:  # noqa: BLE001
        summary.update(
            {
                "status": "failed",

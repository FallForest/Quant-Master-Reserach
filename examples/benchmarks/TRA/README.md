...
@article{yang2020quant_master,
  title={QuantMaster: An AI-oriented Quantitative Investment Platform},
...
**Update**: `TRA` has been moved to `quant_master.contrib.model.pytorch_tra` to support other `QuantMaster` components like  `quant_master.workflow` and `Alpha158/Alpha360` dataset.

Please follow the official [doc](https://quant_master.readthedocs.io/en/latest/component/workflow.html) to use `TRA` with `workflow`. Here we also provide several example config files:
...
The performances of `TRA` are reported in [Benchmarks](https://github.com/microsoft/quant_master/tree/main/examples/benchmarks).
...
* Running from scripts with default parameters
  ...
  You can directly run from QuantMaster command `qrun`:
  ```
...
This result is generated by quant_master==0.7.1.
